...
import { Hash, Users, Lock, X, Loader2 } from "lucide-react";
import { useToast } from "../contexts/ToastContext";
import useAuth from "../hooks/useAuth";

function ModalPreviewChannel({ channelId, workspaceId, onClose, onSuccess }) {
  ...
  const handleJoinChannel = async () => {
    try {
      setIsJoining(true);
      const result = await authFetch(`/api/channels/${channelId}/join`, {
        method: "POST",
      });

      if (result.requestId) {
        // Private channel - request sent
        addToast(result.message, "success");
        onClose();
      } else {
        // Public channel - joined successfully
        addToast(result.message, "success");
        if (onSuccess) {
          onSuccess(channelId);
        }
            ...
            <>
              ...
              <div className="mb-6">
                <div className="mb-3 flex items-center gap-3">
                  <div className="rounded-lg bg-[rgb(30,41,59)]/10 p-2.5">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-[rgb(30,41,59)]">
                      {channelPreview.name}
                    </h3>
                    <div className="flex items-center gap-2 text-xs">
                      {channelPreview.isPrivate ? (
                        ...
                        <span className="rounded bg-green-500/10 px-2 py-0.5 text-green-600 font-medium">
                          Public
                        </span>
                      )}
                    </div>
              ...
              <div className="flex gap-3">
                ...
                {channelPreview.canJoin && !channelPreview.isMember && (
                  <button
                    ...
                    disabled={isJoining}
                    className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-[rgb(30,41,59)] px-4 py-2.5 font-medium text-white transition-all hover:opacity-90 disabled:opacity-50"
                  >
                    {isJoining ? (
                      <>
...
export default ModalPreviewChannel;
